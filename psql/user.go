...
func CreateUser(tx *sql.Tx, username, displayName, avatarURL string, settings *orderedmap.OrderedMap) error {
	...
	query := `
	INSERT INTO users (username, display_name, avatar_url, settings, flag)
	VALUES ($1, $2, $3, $4, 'active')
`
	if _, err := tx.Exec(query, username, displayName, avatarURL, settingsJSON); err != nil {
...
func GetUser(tx *sql.Tx, username string) (*User, error) {
	query := `
	SELECT username, display_name, avatar_url, settings, flag, created_at, updated_at
	FROM users
	WHERE username = $1
`
