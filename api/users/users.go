package users

import (
	"database/sql"
	"encoding/json"
	"fmt"
	"net/http"
	"os"

	tools "github.com/Hana-ame/neo-moonchan/Tools"
	db "github.com/Hana-ame/neo-moonchan/Tools/db/pq"
	"github.com/Hana-ame/neo-moonchan/Tools/orderedmap"
	"github.com/Hana-ame/neo-moonchan/psql"
	"github.com/gin-gonic/gin"
)

func Users(c *gin.Context) {
	username := c.Param("username")
	id := "https://" + os.Getenv("HOST") + "/users/" + username

	// 别用这个,去数据库添加剂啊
	if username == "别用这个,去数据库添加剂啊" {
		s := `
		{
			"@context": [
				"https://www.w3.org/ns/activitystreams",
				"https://w3id.org/security/v1",
				{
					"manuallyApprovesFollowers": "as:manuallyApprovesFollowers",
					"toot": "http://joinmastodon.org/ns#",
					"featured": {
						"@id": "toot:featured",
						"@type": "@id"
					},
					"featuredTags": {
						"@id": "toot:featuredTags",
						"@type": "@id"
					},
					"alsoKnownAs": {
						"@id": "as:alsoKnownAs",
						"@type": "@id"
					},
					"movedTo": {
						"@id": "as:movedTo",
						"@type": "@id"
					},
					"schema": "http://schema.org#",
					"PropertyValue": "schema:PropertyValue",
					"value": "schema:value",
					"discoverable": "toot:discoverable",
					"Device": "toot:Device",
					"Ed25519Signature": "toot:Ed25519Signature",
					"Ed25519Key": "toot:Ed25519Key",
					"Curve25519Key": "toot:Curve25519Key",
					"EncryptedMessage": "toot:EncryptedMessage",
					"publicKeyBase64": "toot:publicKeyBase64",
					"deviceId": "toot:deviceId",
					"claim": {
						"@type": "@id",
						"@id": "toot:claim"
					},
					"fingerprintKey": {
						"@type": "@id",
						"@id": "toot:fingerprintKey"
					},
					"identityKey": {
						"@type": "@id",
						"@id": "toot:identityKey"
					},
					"devices": {
						"@type": "@id",
						"@id": "toot:devices"
					},
					"messageFranking": "toot:messageFranking",
					"messageType": "toot:messageType",
					"cipherText": "toot:cipherText",
					"suspended": "toot:suspended",
					"focalPoint": {
						"@container": "@list",
						"@id": "toot:focalPoint"
					}
				}
			],
			"id": "https://mstdn.work.gd/users/nanakananoka",
			"type": "Person",
			"following": "https://mstdn.work.gd/users/nanakananoka/following",
			"followers": "https://mstdn.work.gd/users/nanakananoka/followers",
			"inbox": "https://mstdn.work.gd/users/nanakananoka/inbox",
			"outbox": "https://mstdn.work.gd/users/nanakananoka/outbox",
			"featured": "https://mstdn.work.gd/users/nanakananoka/collections/featured",
			"featuredTags": "https://mstdn.work.gd/users/nanakananoka/collections/tags",
			"preferredUsername": "nanakananoka",
			"name": "ななかなのか？！",
			"summary": "<p>头像是性癖<br />头图是自己</p>",
			"url": "https://mstdn.work.gd/@nanakananoka",
			"manuallyApprovesFollowers": false,
			"discoverable": false,
			"published": "2024-05-18T00:00:00Z",
			"devices": "https://mstdn.work.gd/users/nanakananoka/collections/devices",
			"alsoKnownAs": [
        		"https://mstdn.jp/users/nanakananika"
    		],	
			"publicKey": {
				"id": "https://mstdn.work.gd/users/nanakananoka#main-key",
				"owner": "https://mstdn.work.gd/users/nanakananoka",
				"publicKeyPem": "-----BEGIN PUBLIC KEY-----\\n-----END PUBLIC KEY-----\n"
			},
			"privateKeyPem":"-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
			"tag": [],
			"attachment": [],
			"endpoints": {
				"sharedInbox": "https://mstdn.work.gd/inbox"
			},
			"icon": {
				"type": "Image",
				"mediaType": "image/png",
				"url": "https://media.mstdn.jp/accounts/avatars/112/461/152/626/039/703/original/f4a814f00c18cd26.png"
			},
			"image": {
				"type": "Image",
				"mediaType": "image/jpeg",
				"url": "https://media.mstdn.jp/accounts/headers/112/461/152/626/039/703/original/9f5bab767f21a76b.jpg"
			}
		}`
		o := orderedmap.New()
		json.Unmarshal([]byte(s), &o)
		fmt.Println(o.Keys())
		c.Header("Content-Type", "application/activity+json; charset=utf-8")
		c.JSON(http.StatusOK, o)
		return
	}

	var o *orderedmap.OrderedMap

	err := db.Exec(func(tx *sql.Tx) error {
		user, err := psql.ReadUser(tx, id)
		if err != nil {
			return err
		}

		o = user

		return tx.Commit()
	})
	if err != nil {
		c.Header("X-Error", err.Error())
		c.JSON(http.StatusInternalServerError, err.Error())
		return
	}

	o.Delete("privateKeyPem")

	tools.MoveToFirstInPlace(o.Keys(), "@context")
	c.Header("Content-Type", "application/activity+json; charset=utf-8")
	c.JSON(http.StatusOK, o)
}
